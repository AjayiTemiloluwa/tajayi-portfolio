 ...
 * rows gliding in from the right with staggered sepia nodes.
 */
    ...
    <section id="experience" className="relative scroll-mt-20 overflow-hidden hairline-top">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
        ...
        <h2 className="mt-4 font-display text-3xl font-bold tracking-tight text-parchment sm:text-5xl">
          <MaskText segments={[{ text: 'Where the bias for ' }, { text: 'action', sepia: true }, { text: ' was built.' }]} delay={0.08} />
        </h2>

        <ol className="relative mt-12 space-y-10">
          {/* the sepia spine */}
          <span
            className="absolute bottom-2 left-[7px] top-2 w-px bg-gradient-to-b from-sepia/50 via-sepia/25 to-transparent sm:left-[9px]"
            aria-hidden="true"
          />
              ...
              <span className="absolute left-0 top-1.5 sm:top-2" aria-hidden="true">
                {role.current ? (
                  <span className="relative flex h-4 w-4 sm:h-5 sm:w-5">
                    ...
                    <span className="relative inline-flex h-4 w-4 items-center justify-center rounded-full border border-sepia bg-moss text-ink sm:h-5 sm:w-5">
                      <Briefcase className="h-2.5 w-2.5" />
                    </span>
                  </span>
                ) : (
                  <span className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-sepia/60 bg-[var(--bg)] sm:h-5 sm:w-5">
                ...
                <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-fg/60">
                      <span className="font-medium text-sepia">{role.org}</span> · {role.place}
                    </p>
                  </div>
                ...
                <ul className="mt-4 space-y-2.5">
                  {role.bullets.map((b) => (
                    <li key={b.slice(0, 32)} className="flex gap-3 text-sm leading-relaxed text-fg/65">
                      <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-sepia/70" aria-hidden="true" />
                      {b}
                    </li>
                  ))}
                </ul>
