...
function Row({ item, index, active, setActive }: { item: WorkItem; index: number; active: number | null; setActive: (i: number | null) => void }) {
  ...
  return (
    <motion.article
      ...
      className={`group relative overflow-hidden rounded-xl p-6 transition-all duration-300 hover:-translate-y-0.5 sm:p-7 ${featured ? 'border-beam glass-gold' : 'glass'} ${active === index ? 'gold-outline' : ''}`}
      data-cursor="VIEW"
    >
      <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-2">
        <div className="flex min-w-0 items-start gap-5">
          <span className="mt-1.5 font-mono text-sm font-bold tabular-nums text-gold/70">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 font-mono text-[11px] uppercase tracking-[0.18em] text-fg/50">
              {item.org}
              {featured && <span className="ml-3 rounded-full bg-gold/15 px-2 py-0.5 text-[9px] font-bold text-gold">Featured</span>}
            </p>
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-fg/60">{item.summary}</p>
            <ul className="mt-4 flex flex-wrap gap-2">
              {item.stack.map((s) => (
                <li key={s} className="rounded-full border border-gold/20 px-2.5 py-1 font-mono text-[10px] text-gold/90">{s}</li>
              ))}
            </ul>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-3">
          ...
          <span className="glass inline-flex h-9 w-9 items-center justify-center rounded-full text-fg/50 transition group-hover:border-gold group-hover:text-gold">
    ...
    <section id="work" ref={sectionRef} onMouseMove={onMove} onMouseLeave={() => setActive(null)} className="relative scroll-mt-20 overflow-hidden hairline-top bg-mist/40">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-4 font-display text-3xl font-bold tracking-tight text-parchment sm:text-5xl">
              <MaskText segments={[{ text: 'Things I’ve built that ' }, { text: 'decide.', gold: true }]} />
            </h2>
          </div>
          <motion.p initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.25 }} className="max-w-sm text-sm leading-relaxed text-fg/55">
            Systems shipped at work, school and in between — each one measured by what it saved, surfaced or settled.
          </motion.p>
        </div>
      ...
      <AnimatePresence>
        {canHover && active !== null && (
          ...
          >
            <div className="border-beam glass-gold rounded-xl p-5 shadow-2xl shadow-black/40">
              ...
              <ul className="mt-3 flex flex-wrap gap-1.5">
                {work[active].stack.slice(0, 4).map((s) => (
                  <li key={s} className="rounded-full bg-gold/10 px-2 py-0.5 font-mono text-[9px] text-gold/90">{s}</li>
                ))}
              </ul>
            </div>
          </motion.div>
