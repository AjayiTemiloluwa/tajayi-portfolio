...
export type MaskSegment = { text: string; sepia?: boolean }

/**
 * MaskText — Robin Noguier-style masked reveal: every word rises out of
 * its own overflow mask when it scrolls into view. `sepia: true` words get
...
export function MaskText({
  ...
  const words: { w: string; sepia: boolean; i: number }[] = []
  let i = 0
  for (const seg of segments) {
    for (const w of seg.text.split(' ')) {
      if (w === '') continue
      words.push({ w, sepia: !!seg.sepia, i: i++ })
    }
  }
  ...
  return (
    <span ref={ref} className={className} data-noreveal aria-label={words.map((x) => x.w).join(' ')}>
      {words.map(({ w, sepia, i }) => (
        ...
        >
          <motion.span
            className={`inline-block will-change-transform ${sepia ? 'text-sepia-grad italic' : ''}`}
