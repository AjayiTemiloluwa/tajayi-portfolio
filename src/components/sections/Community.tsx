    ...
    <section id="community" className="relative scroll-mt-20 overflow-hidden hairline-top bg-mist/40">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
        ...
        <h2 className="mt-4 font-display text-3xl font-bold tracking-tight text-parchment sm:text-5xl">
          <MaskText segments={[{ text: 'People grow ' }, { text: 'because', sepia: true }, { text: ' someone showed up.' }]} delay={0.08} />
        </h2>
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-3">
          {leadership.map((l, i) => (
            <motion.div key={l.org} {...zoom(i * 0.09)} className="spotlight-card glass tilt-card h-full rounded-xl p-6">
              ...
              <p className="mt-4 font-mono text-[11px] text-sepia/80">{l.period}</p>
            </motion.div>
          ))}
        </div>
        ...
        <motion.ul
          ...
          className="mt-6 grid gap-px overflow-hidden rounded-xl hairline bg-sepia/10 sm:grid-cols-2"
        >
          {honors.map((h) => (
            <motion.li
              key={h.title}
            ...
            >
              ...
              <span className="shrink-0 rounded-full bg-sepia/15 px-2.5 py-1 font-mono text-[10px] font-bold text-sepia">{h.year}</span>
            </motion.li>
          ))}
        </motion.ul>
      </div>
